...
function _getReplyDetails(details, markup) {
    ...
    $("div.reply_options")
        .find("b")
        .each(function(i, element) {
            var infoType = $(element)
                .text()
                .trim(); // set contact name
            ...
            if (/call/i.test(infoType)) {
                $(element)
                    ...
                    .each(function(i, li) {
                        var value = $(li)
                            .text()
                            .trim(); // check for phone value (based on the emoji)
...
function _getRequestOptions(client, options, query) {
    ...
    requestOptions.hostname = [
        _core.default.Validation.coalesce(options.city, client.options.city, ""), // introducing fix for #7
        _core.default.Validation.coalesce(options.baseHost, client.options.baseHost, DEFAULT_BASE_HOST)
    ].join("."); // preserve any extraneous input option keys (may have addition instructions for underlying request object)

    (0, _keys.default)(options).forEach(function(key) {
        if (
            !QUERY_KEYS.indexOf(key) &&
            _core.default.Validation.isEmpty(requestOptions[key]) &&
            _core.default.Validation.isEmpty(DEFAULT_REQUEST_OPTIONS[key])
        ) {
    ...
    requestOptions.path = [
        requestOptions.path,
        _core.default.Validation.coalesce(options.category, DEFAULT_CATEGORY)
    ].join(""); // setup querystring
    ...
    if (options.vin) {
        requestOptions.path = [requestOptions.path, QUERY_PARAM_VIN, options.vin].join("");
    } // add vin
